...
jq -c -S --null-input --slurpfile gemarkungen ./temp/gemarkungen.txt --slurpfile fluren ./temp/fluren.txt 'reduce $gemarkungen[] as $i ({}; setpath([$i.gmdschl, $i.name];{schluessel: $i.schluessel, fluren: [$fluren[] | select(.schluessel == $i.schluessel).name]}))' > ./data/gemarkungen_fluren_nrw.json
md5sum ./data/gemarkungen_fluren_nrw.json > ./data/gemarkungen_fluren_nrw.json.md5
rm -r temp