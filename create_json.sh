...
jq -c --argfile katasteraemter katasteraemter.json '.features[] | select(.properties.art|contains("Gemarkungsteil/Flur")|not ) | {name: (.properties.name + " (" + (.properties.schluessel | tostring | .[2:6]) +")"), schluessel: .properties.schluessel | tostring | .[2:6], gmdschl: .properties.gmdschl | tostring | .[1:5]} | .gmdschl |= $katasteraemter[.]' ./temp/data.geojson > ./temp/gemarkungen.txt
jq -c '.features[] | select(.properties.art|contains("Gemarkungsteil/Flur")) | {name: .properties.name | tonumber | tostring, schluessel: .properties.schluessel | tostring | .[2:6]}' ./temp/data.geojson > ./temp/fluren.txt
